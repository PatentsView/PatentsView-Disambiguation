...
def create_uuid_map(config, source='granted_patent_database'):
    ...
    query = "SELECT {id_field}, {document_id_field}, {sequence_field} FROM {db}.rawassignee ra {filter} and " \
            "ra.organization like '%tech%'".format(
...
def generate_uuid_map(config):
    ...
    output_file = f"{config['BASE_PATH']['assignee']}".format(end_date=end_date,
                                                              data_root=config['FOLDERS']['data_root']) + \
                  config['ASSIGNEE_UPLOAD']['uuidmap']
    print(output_file)
